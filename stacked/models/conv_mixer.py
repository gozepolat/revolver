...
def make_scoped(scope, class_type, *args, **kwargs):
    prefix = scope
    suffixes = [str(s) for s in args] + [f'{str(k)},{str(v)}' for k, v in kwargs.items()]
    suffix = '_'.join(suffixes)
    scope = f'{prefix}__{suffix}'
    if scope in GLOBAL_PARAMS:
        print(f'{scope} already exists')
        return GLOBAL_PARAMS[scope]
    instance = class_type(*args, **kwargs)
    GLOBAL_PARAMS[scope] = instance
    return instance
    ...
    return nn.Sequential(
        ...
        *[nn.Sequential(
            Residual(nn.Sequential(
                make_scoped('conv', nn.Conv2d,
                            dim, dim, kernel_size,  # args
                            groups=dim, padding="same"),  # kwargs
                nn.GELU(),
                nn.BatchNorm2d(dim)
            )),

            make_scoped('conv1x1', nn.Conv2d, dim, dim, kernel_size=1),
            nn.GELU(),
            nn.BatchNorm2d(dim)
...
print(f'\nTotal number of parameters in model: {n_parameters}')
